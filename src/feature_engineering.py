    ...
    def transform(self, df):
        ...
        for feature in ['Tenure', 'Salary', "Average Monthly Hours Worked"]:
            for role in job_roles_features:
                new_cols[role + "_" + feature] = df[role] * df[feature]
        ...
        for features in [martial_status, work_life_balance, educational_level]:
            for feature in features:
                new_cols["Average Monthly Hours Worked_" + feature] = df['Average Monthly Hours Worked'] * df[feature]

        new_cols['Average Monthly Hours Worked and Distance from Home'] = df['Average Monthly Hours Worked'] + df[
            'Distance from Home']
